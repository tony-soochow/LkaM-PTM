...
from evaluate_code import caculate_metric
...
class PLMEncoder(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        pooled_output = self.bert(input_ids=input_ids, attention_mask=attention_mask, return_dict=False)
        ...
        return prot_out
class model1_Bilstm_transformer_MCNN(torch.nn.Module):
    def __init__(self,Bilstm_input_feature_size_1,Bilstm_output_feature_size_1,Bilstm_layer_number_1,n_layers_1,n_head_1,d_k_1,CNN_out_channels_1,conv1_kernel_size_1,conv1_padding_1   ,max_len_1  , vocab_size_1  ,device_1 ):
        super(model1_Bilstm_transformer_MCNN,self).__init__()
        ...
        # self.layers = torch.nn.ModuleList([EncoderLayer(Bilstm_output_feature_size*2, n_head, d_ff, 0.5) for n_l in range(n_layers)])
        self.blocknew=BlockNew(Bilstm_output_feature_size*2,3,Bilstm_output_feature_size,Bilstm_output_feature_size*2)
    ...
    def forward(self, input_ids,x6):
        ...
        feature_learn_model1_Bilstm_transformer_MCNN =self.feature_learn(x)
        return feature_learn_model1_Bilstm_transformer_MCNN,attention_weights
class SparseAutoencoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, sparsity_constraint=0.05):
...
class StackedSparseAutoencoder(nn.Module):
    def __init__(self, input_dim):
        ...
        self.layer1 = SparseAutoencoder(input_dim, input_dim // 4)
        self.layer2 = SparseAutoencoder(input_dim // 4, input_dim // 16)
    def forward(self, x):
        x_hat,z1 = self.layer1(x)
        x_hat2,z2 = self.layer2(z1)
        return x_hat2  # 返回低维表示和损失
class combine_model(nn.Module):
    def __init__(self,BERT_encoder, Bilstm_input_feature_size_1, Bilstm_output_feature_size_1, Bilstm_layer_number_1, n_layers_1,
        ...
        self.model1_Bilstm_transformer_MCNN=model1_Bilstm_transformer_MCNN(Bilstm_input_feature_size_1, Bilstm_output_feature_size_1, Bilstm_layer_number_1, n_layers_1,
    ...
    def forward(self, x1,x2,x3,x4,x5):
        x_2,atts = self.model1_Bilstm_transformer_MCNN(x2,x1)
        ...
        return logit,feature_learn_combine
...
def test(model: torch.nn.Module, test_loader, loss_function, device, show):
    ...
    with torch.no_grad():
        for idx, (*x, y) in tqdm(enumerate(test_loader), disable=not show, total=len(test_loader)):
            ...
            x[3] = x[3].to(device)
            x[4] = x[4].to(device)
            # x[5] =( x[5].float()).to(device)
            y = (y.float()).to(device)
            class_fenlei,representation = model(*x)
    ...
    return evaluation,repres_list,label_list,roc_data,aupr_data,true_sample_label, pro_1