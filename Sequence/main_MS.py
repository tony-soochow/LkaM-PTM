...
from manage.MyDataset_test import MyDataset_test
...
print('****************Multi-scale Sequence Representation Learning************** ')
phase=0
K_FOLD=5
best_metrics_list = []
val_metrics_list=[]
#########################Motif-based information extraction#########################
for fold in range(K_FOLD):
    ...
    tokenizer = AutoTokenizer.from_pretrained('C:/Users/lmj/Desktop/Contrast/LMCrot-main/prot_t5_xl_uniref50', do_lower_case=False, use_fast=False)
    BERT_encoder = AutoModel.from_pretrained('C:/Users/lmj/Desktop/Contrast/LMCrot-main/prot_t5_xl_uniref50', local_files_only=True, output_attentions=False).to(device)
    n_epoch=20
    embedding_dim=128
    ...
    data_loaders = {phase_name: DataLoader(MyDataset_test(data_path, phase_name,file_name, sequence_center, windowsize, PTM_name,tokenizer,True    ,train_structure_feature,val_structure_feature,test_structure_feature   ),
                                                                            batch_size=batch_size,
                                                                            pin_memory=True,
                                                                            shuffle=False)
                                                                             for phase_name, file_name in file_names.items()}

    #########################Context sequence information extraction#########################
    print('Please wait, this process will take about ten minutes...............................................')
    ...
    for epoch in range(1, n_epoch + 1):
        ...
        for idx, (*x,y) in tbar:
            print(np.array(x[0]).shape,np.array(x[1]).shape,np.array(x[2]).shape,np.array(x[3]).shape,np.array(x[4]).shape,np.array(x[5]).shape)
            model.train()
        ...
        for _p in ['Train_data']:
            performance, repres_list, label_list, _, _,train_true_label,train_pre_label = test(model, data_loaders[_p], loss_function, device, True)
            if _p == 'Train_data':
        ...
        for _p in ['Val_data']:
            performance, repres_list, label_list, _, _,val_true_label,val_pre_label = test(model, data_loaders[_p], loss_function, device, True)
        ...
        print("Epoch：", epoch - 1)
        print(f'train_acc:{train_acc_record[epoch - 1]},val_acc:{val_acc_record[epoch - 1]},train_loss:{train_losses[epoch-1]},val_loss:{val_losses[epoch-1]}')
        for _p in ['Test_data']:
            metric, repres_list_test, label_list, roc, aupr, test_true_label, pre_label = test(model, data_loaders[_p], loss_function, device, True)
    ...
    torch.save(model, f'save_models/{PTM_name}/model_fold_{fold}.pth')  # 保存模型和优化器状态
    best_metrics_list.append(best_metrics)
    val_metrics_list.append(val_metrics)
val_metrics_df = pd.DataFrame(val_metrics_list)
val_metrics_df.to_csv(f'{PTM_name}_result/val_metrics_5fold.csv',float_format='%.4f', index=False)
best_metrics_df = pd.DataFrame(best_metrics_list)
best_metrics_df.to_csv(f'{PTM_name}_result/best_metrics_5fold.csv',float_format='%.4f', index=False)
