...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.4, linear=False):
        super().__init__()
...
class AttentionModule(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv0 = nn.Conv1d(dim, dim, 9, padding=4, groups=dim)
        self.conv_spatial = nn.Conv1d(dim, dim, 7, stride=1, padding=15, groups=dim, dilation=5)
    def forward(self, x):
        u = x.clone()
        attn = self.conv0(x)
